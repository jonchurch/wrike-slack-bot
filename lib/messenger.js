...
class Messenger {
    ...
    handleText(bot, msg) {
        ...
        if (inputParser.saidWrike(text)) {
            return handlers.tasks.getAll(bot, msg)
        }
        if (inputParser.saidContacts(text)) {
            return handlers.casual.getAllContacts(bot, msg)
        }
        if (inputParser.saidActive(text)) {
            console.log('MESSENGER HEARD ACTIVE')
            let id = process.env.MY_WRIKE_ID
            return handlers.tasks.getActiveByUser(bot, msg, id)
        }
    }
}
